...
| **Feature settings** | Rewrite `settings_screen.dart`; add `account_edit_screen.dart` |
| **Feature wechat** | Add `wechat_bind_screen.dart` (alongside drafts) |
| **UI / theme** | Reuse `AppColors` / `AppTheme`; no new third-party UI kit |
...
5. **Account binding**
   - Email: show fake email +「已绑定」status
   - WeChat:「已绑定」+ coral「解绑」→ toast (e.g.「解绑功能即将开放」); tapping the WeChat row (or a clear entry affordance) navigates to `/settings/wechat-bind` so the bind UI can be previewed
...
**AppBar:** Leading back; trailing coral「保存」→ toast「已保存」(optional `pop`).
...
- Bottom full-width coral button「保存修改」→ same save toast; may `pop` (does **not** write back into settings placeholder state this iteration).
...
| Theme toggle | Immediate theme change via provider |
...
- [ ] Dark mode toggles app theme and persists across restart
- [ ] Sync switches and threshold sliders move locally
- [ ] Edit → account page (no bottom nav); save / avatar toasts; back returns
- [ ] WeChat row → bind page; paste / clear / confirm (empty vs filled); back returns
