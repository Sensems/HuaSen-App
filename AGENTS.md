...
- Use shared `CustomAppBar` (left-aligned titles) on note editor, note detail, and account edit — keep those headers consistent with other app pages.
...
- Notes list must query only `type=PUBLISHED`; IMAGE drafts should render the image from `meta.media_url` in the card content area (not text placeholder only); draft card edit/delete actions stay left-aligned at intrinsic/fixed width (not half-width Expanded), and delete requires a confirmation dialog; published cards should show a ≤2-line content excerpt plus right-aligned colored file-type icons on the time row.
...
- Git remote: `https://github.com/Sensems/HuaSen-App.git` (Sensems/HuaSen-App); Flutter app lives at the repo root (no nested `sebhua_notes_app/` app directory).
